				...
				const struct i2c_device_id *id)
{
	...
	err = pcf8563_read_block_data(client, PCF8563_REG_ST1, 9, buf2);
	if (err < 0)
		return err;
	if (buf2[PCF8563_REG_SC] & PCF8563_SC_LV) {
		dev_err(&client->dev, "low voltage detected\n");
		err = pcf8563_set_datetime(client, &tm);
		if (err < 0)
			return err;
		pcf8563->voltage_low = 0;
	}
	pcf8563_get_datetime(client, &tm_read);
	if (((tm_read.tm_year < 70) | (tm_read.tm_year > 137)) | (tm_read.tm_mon == -1) | (rtc_valid_tm(&tm_read) != 0)) {
		pcf8563_set_datetime(client, &tm); //initialize the at8563
	}
