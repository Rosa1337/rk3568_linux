...
struct ov5695 {
	...
	bool			power_on;
	struct ov5695_otp_info_r1a *otp_r1a;
	struct ov5695_otp_info_r2a *otp_r2a;
	u32			module_index;
...
static int __ov5695_power_on(struct ov5695 *ov5695)
{
	...
	if (!IS_ERR(ov5695->power_gpio))
		gpiod_set_value_cansleep(ov5695->power_gpio, 1);

	usleep_range(1000, 2000);

	if (!IS_ERR_OR_NULL(ov5695->pins_default)) {
		ret = pinctrl_select_state(ov5695->pinctrl,
					   ov5695->pins_default);
		if (ret < 0)
...
static void __ov5695_power_off(struct ov5695 *ov5695)
{
	...
	if (!IS_ERR(ov5695->reset_gpio))
		gpiod_set_value_cansleep(ov5695->reset_gpio, 1);
	if (!IS_ERR_OR_NULL(ov5695->pins_sleep)) {
		ret = pinctrl_select_state(ov5695->pinctrl,
					   ov5695->pins_sleep);
		if (ret < 0)
				  ...
				  struct i2c_client *client)
{
	...
	dev_err(dev, "ov5695_check_sensor_id=0x%x\n", id);   
	if (id != CHIP_ID) {
...
static int ov5695_parse_of(struct ov5695 *ov5695)
{
	struct device *dev = &ov5695->client->dev;
	...
	ov5695->lane_num = rval;
	/*
	if (4 == ov5695->lane_num) {
		ov5695->cur_mode = &supported_modes_r2a_4lane[0];
		supported_modes = supported_modes_r2a_4lane;
		ov5695->cfg_num = ARRAY_SIZE(supported_modes_r2a_4lane);
		ov5695->pixel_rate = MIPI_FREQ * 2U * ov5695->lane_num / 10U;
		dev_info(dev, "lane_num(%d)  pixel_rate(%u)\n",
				 ov5695->lane_num, ov5695->pixel_rate);
	} else {
		ov5695->cur_mode = &supported_modes_r2a_2lane[0];
		supported_modes = supported_modes_r2a_2lane;
		ov5695->cfg_num = ARRAY_SIZE(supported_modes_r2a_2lane);
		ov5695->pixel_rate = MIPI_FREQ * 2U * (ov5695->lane_num) / 10U;
		dev_info(dev, "lane_num(%d)  pixel_rate(%u)\n",
				 ov5695->lane_num, ov5695->pixel_rate);
	}*/
			...
			const struct i2c_device_id *id)
{
	...
	ret = ov5695_parse_of(ov5695);
	if (ret != 0)
		return -EINVAL;

	ov5695->pinctrl = devm_pinctrl_get(dev);
	if (!IS_ERR(ov5695->pinctrl)) {
		ov5695->pins_default =
			pinctrl_lookup_state(ov5695->pinctrl,
					     OF_CAMERA_PINCTRL_STATE_DEFAULT);
		if (IS_ERR(ov5695->pins_default))
			dev_err(dev, "could not get default pinstate\n");

		ov5695->pins_sleep =
			pinctrl_lookup_state(ov5695->pinctrl,
					     OF_CAMERA_PINCTRL_STATE_SLEEP);
		if (IS_ERR(ov5695->pins_sleep))
